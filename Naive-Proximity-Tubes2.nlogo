...
patches-own [
  pass-matrix                           ;;4x4 binary matrix consisting whether a patch
                                        ;; has way for L turtles to pass from (left, right,
                                        ;; up, down) for specific headings (0, 90, 180, 270);
                                        ;; Row for direction, col for heading
  assessed?
  visited?
  visit-count
  absolute-goal-dist
...
to setup-patches
  ask patches [
    set pcolor black
    set pass-matrix matrix:make-constant 4 4 0
    set visited? false
    set visit-count 0
    set absolute-goal-dist 0
    set assessed? false
  ]
end
...
to set-goal
  if mouse-down? [
    ...
    ask patches [
      set absolute-goal-dist distance-from-to pxcor pycor goal-x goal-y
    ]
  ]
end
...
to assess-patchs
  if not (pcolor = white) [
    ; vertical direction
    if is-valid-path (pxcor + 1) pycor [
      if is-valid-path (pxcor - 1) pycor[
        matrix:set pass-matrix 2 1 1
        matrix:set pass-matrix 3 1 1
        matrix:set pass-matrix 2 3 1
        matrix:set pass-matrix 3 3 1
      ]
      matrix:set pass-matrix 2 0 1
      matrix:set pass-matrix 3 0 1
    ]
    if is-valid-path (pxcor - 1) pycor[
      matrix:set pass-matrix 2 2 1
      matrix:set pass-matrix 3 2 1
    ]
    ; horizontal direction
    if is-valid-path pxcor (pycor + 1)[
      if is-valid-path pxcor (pycor - 1)[
        matrix:set pass-matrix 0 0 1
        matrix:set pass-matrix 1 0 1
        matrix:set pass-matrix 0 2 1
        matrix:set pass-matrix 1 2 1
      ]
      matrix:set pass-matrix 0 3 1
      matrix:set pass-matrix 1 3 1
    ]
    if is-valid-path pxcor (pycor - 1)[
      matrix:set pass-matrix 0 1 1
      matrix:set pass-matrix 1 1 1
    ]
  ]
end
...
to-report pass-check
  ...
  show paths
  report patches with [member? self paths]
end

to-report corner-check [coor-set]
  report coor-set
end

;;accepts agentset of patches
to-report least-visited [coor-set]
  report coor-set with-min [visit-count]
end

;;accepts agentset of patches
to-report minimum-path [coor-set]
  report min-one-of coor-set [absolute-goal-dist]
end
...
true
8
Rectangle -11221820 true true 60 15 150 285
Rectangle -11221820 true true 150 195 240 285
